...
def SchoolEnrollments(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                form.add_error(
                    None,
                    "An enrollment with the same championship, sport, age, and gender already exists."
                )
        else:
    ...
    return render(request, "enrollments/school_enrolls.html", {"form": form, "enrollments": enrollments})
...
def AllEnrollments(request):
    ...
    return render(request, "enrollments/School_enrollments.html", context)
